"""Parallel-beam reconstruction from (simulated) real CT data.

Real-data analogue of ``fbp_parallel.py``. Instead of treating the
forward projection output as "the sinogram" and feeding it straight
into FBP, this script runs the line integrals through the full X-ray
detection chain:
...
The resulting ``sino_hat`` is the kind of sinogram a real parallel-
beam detector pipeline produces after flat-field / air-scan
normalisation. It is fed into the usual analytical FBP pipeline
unchanged.
...
def main():
    ...
    # ------------------------------------------------------------------
    # 2. Source trajectory (parallel beam, full 2*pi scan)
    # ------------------------------------------------------------------
    num_angles = 360
    ...
    sinogram_clean = ParallelProjectorFunction.apply(
        image_torch,
        angles_torch,
        num_detectors,
        detector_spacing,
    ...
    # ``mu_scale`` is a typical soft-tissue linear attenuation
    # coefficient (1/mm) at ~70 keV. Multiplying the dimensionless
    # phantom by ``mu_scale`` turns it into a physical mu-map so that
    # Beer-Lambert yields realistic photon counts.
    mu_scale = 0.02           # 1/mm, water-like at ~70 keV
    # ``I0`` is the unattenuated photon count per detector cell per
    # view (what the detector would see if nothing was in the beam).
    # Clinical X-ray CT is typically 1e4 - 1e6 per bin depending on
    # dose; lowering I0 increases Poisson noise and is what drives the
    # "low-dose CT" reconstruction problem.
    I0 = 1.0e4
    # Seed the Poisson generator so re-runs reproduce exactly.
    rng_seed = 0
    # Floor the clamped photon count to avoid ``-log(0) = inf`` at
    # heavily-attenuated rays. ``0.5`` is a common workaround; more
    # sophisticated pipelines fit a statistical model to zero-count
    # bins.
    eps_intensity = 0.5

    # --- 6.1  Attach a physical attenuation scale --------------------
    # ``sinogram_clean`` is in "phantom_value * voxel" units; scaling
    # by ``mu_scale * voxel_spacing`` converts it into the dimensionless
    # ``sum(mu * dl)`` that lives in Beer-Lambert's exponent.
    ...
    # Photon arrivals follow Poisson statistics. Heavily-attenuated
    # rays collect very few photons, so their relative noise
    # ``~1/sqrt(N)`` blows up - that is what drives the "streak"
    # artefacts that bright bones and metal create in real CT
    # reconstructions.
    ...
    # "phantom_value * voxel" units that ``parallel_weighted_backproject``
    ...
    # --- 7.1  1D ramp filter along the detector axis -----------------
    ...
    print("Parallel Beam FBP example (simulated real-data workflow):")
