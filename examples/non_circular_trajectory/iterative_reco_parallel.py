...
from diffct.geometry import (sinusoidal_trajectory_2d_parallel, custom_trajectory_2d_parallel)


def custom_wobble_trajectory_rays(angles):
    """Custom trajectory: Wobbling ray directions."""
    dirs = torch.zeros((len(angles), 2), device=angles.device, dtype=angles.dtype)
    wobble_amplitude = 0.1
    wobble = wobble_amplitude * torch.sin(3 * angles)
    adjusted_angles = angles + wobble
    dirs[:, 0] = torch.cos(adjusted_angles)
    dirs[:, 1] = torch.sin(adjusted_angles)
    return dirs


def custom_wobble_trajectory_origins(angles):
    """Custom trajectory: Wobbling detector origins."""
    origins = torch.zeros((len(angles), 2), device=angles.device, dtype=angles.dtype)
    offset = 30.0 * torch.sin(2 * angles)
    origins[:, 0] = -offset * torch.sin(angles)
    origins[:, 1] = offset * torch.cos(angles)
    return origins
...
class IterativeRecoModel(nn.Module):
    def __init__(self, image_shape, ray_dir, det_origin, det_u_vec, n_det, det_spacing, voxel_spacing):
        super().__init__()
...
class Pipeline:
    def __init__(self, lr, image_shape, ray_dir, det_origin, det_u_vec,
                 n_det, det_spacing, voxel_spacing,
                 device, epoches=1000):
        self.epoches = epoches
        self.model = IterativeRecoModel(image_shape, ray_dir, det_origin, det_u_vec,
...
def run_reconstruction(trajectory_name, ray_dir, det_origin, det_u_vec,
    ...
    print(f"Ray direction range: x=[{ray_dir[:, 0].min():.2f}, {ray_dir[:, 0].max():.2f}], "
          f"y=[{ray_dir[:, 1].min():.2f}, {ray_dir[:, 1].max():.2f}]")
    print(f"Detector origin range: x=[{det_origin[:, 0].min():.2f}, {det_origin[:, 0].max():.2f}], "
          f"y=[{det_origin[:, 1].min():.2f}, {det_origin[:, 1].max():.2f}]")
...
def main():
    ...
    # 2. Wobbling Trajectory (Custom)
    print("\n" + "="*60)
    print("Generating Wobbling (Custom) Trajectory...")
    print("="*60)
    ray_dir, det_origin, det_u_vec = custom_trajectory_2d_parallel(
        n_views=num_views,
        ray_dir_fn=custom_wobble_trajectory_rays,
        det_origin_fn=custom_wobble_trajectory_origins,
        device=device
    )
    loss_values, reco = run_reconstruction(
        "Wobbling", ray_dir, det_origin, det_u_vec,
        phantom_torch, n_det, det_spacing, voxel_spacing, device, epoches
    )
    results['Wobbling'] = (loss_values, reco)

    # Plot results
    fig = plt.figure(figsize=(15, 10))

    trajectory_names = ['Sinusoidal', 'Wobbling']
    ...
    plt.tight_layout()
    plt.savefig("parallel_trajectory_reconstruction.png", dpi=300, bbox_inches='tight')
    plt.show()
