"""Utility script to visualise trajectory generators from diffct_mlx.geometry.
...
from diffct_mlx import geometry
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--n-views", type=int, default=360,
        help="Number of projection views to sample.",
    )
    parser.add_argument(
        "--sid", type=float, default=1000.0,
        help="Source-to-isocenter distance.",
    )
    parser.add_argument(
        "--sdd", type=float, default=1500.0,
        help="Source-to-detector distance.",
    )
    parser.add_argument(
        "--start-angle", type=float, default=0.0,
        help="Starting angle in radians.",
    )
    parser.add_argument(
        "--end-angle", type=float, default=None,
        help="Optional end angle in radians (defaults to a full rotation when omitted).",
    )
    parser.add_argument(
        "--z-range", type=float, default=100.0,
        help="Axial extent for spiral trajectories.",
    )
    parser.add_argument(
        "--n-turns", type=float, default=2.0,
        help="Number of turns for spiral trajectories.",
    )
    parser.add_argument(
        "--amplitude", type=float, default=50.0,
        help="Amplitude for sinusoidal trajectories.",
    )
    parser.add_argument(
        "--frequency", type=float, default=2.0,
        help="Frequency for sinusoidal trajectories.",
    )
    parser.add_argument(
        "--z-amplitude", type=float, default=50.0,
        help="Vertical amplitude for saddle trajectories.",
    )
    parser.add_argument(
        "--radial-amplitude", type=float, default=30.0,
        help="Radial amplitude for saddle trajectories.",
    )
    parser.add_argument(
        "--sid-std", type=float, default=0.0,
        help="SID variation for random trajectories.",
    )
    parser.add_argument(
        "--pos-std", type=float, default=0.0,
        help="Positional noise for random trajectories.",
    )
    parser.add_argument(
        "--angle-std", type=float, default=0.0,
        help="Angular noise for random trajectories.",
    )
    parser.add_argument(
        "--seed", type=int, default=None,
        help="Random seed for random trajectories.",
    )
    parser.add_argument(
        "--sample-step", type=int, default=25,
        help="Stride for plotting detector orientation arrows.",
    )
    parser.add_argument(
        "--arrow-scale", type=float, default=50.0,
    ...
    parser.add_argument(
        "--show-detectors", action="store_true",
...
def _generate_plot(
    ...
    *,
    ...
    kwargs = _build_kwargs(param_values, param_map)
    src_pos, det_center, det_u_vec, det_v_vec = fn(**kwargs)

    # Convert MLX arrays to numpy for plotting
    src = np.array(src_pos)
    det = np.array(det_center)
    det_u = np.array(det_u_vec)
    det_v = np.array(det_v_vec)
...
def _run_all() -> None:
    gallery_dir = Path(__file__).resolve().parents[1] / "plots"
